    ...
    def run_test(self):
        ...
        with open('iris_test.txt') as f:
            for line in f:
                ...
                try:
                    ...
                    print(f"Conversion error in line: {line.strip()}")
                    print(f"Cannot convert to float: {vec[:-1]}")
                    raise e 
        ...
        for i in range(len(all_vec)):
            ...
            if res == res_col[i]:
                ...
                print("Incorrect case: result: ", res, " expected result: ", res_col[i]) 

        print("Correct answers: ", good_answ)
        print("Accuracy: ", good_answ / len(all_vec))
